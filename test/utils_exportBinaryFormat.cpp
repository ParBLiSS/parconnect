...
/**
 * @file    utils_exportBinaryFormat.cpp
 * @ingroup 
 * @author  Chirag Jain <cjain7@gatech.edu>
 * @brief   Writes the edgeList into the binary format 
 *
...
int main(int argc, char** argv)
{
  ...
  LOG_IF(!comm.rank(), INFO) << "This executable exports graph into binary edgeList files.";

  //Parse command line arguments
  ArgvParser cmd;

  cmd.setIntroductoryDescription("This executable exports graph into binary edgeList files");
  ...
  cmd.defineOption("relabel", "relabel the vertices for contiguity or not (y/n)", ArgvParser::OptionRequiresValue | ArgvParser::OptionRequired);
  ...
  conn::graphGen::writeEdgeListBinaryFormat(edgeList, outputPath, comm);

  comm.barrier();

  LOG_IF(!comm.rank(), INFO) << "Files written to folder " << outputPath;
  LOG_IF(!comm.rank(), INFO) << "You can join these files in any order";
